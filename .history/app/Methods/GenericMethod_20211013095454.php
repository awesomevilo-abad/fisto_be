...
class GenericMethod{
    ...
    $utilities_receipt_no){
        $transactions = DB::select( DB::raw('SELECT * FROM `transactions` WHERE
        company_id = 1 AND
        supplier_id = 2 AND
        utilities_category = 'Electric' AND
        utilities_account_no = '20-0325-25' AND
        (
            (utilities_from >= '2021-11-01' AND utilities_from <= '2021-11-31')
                OR
            (utilities_to >= '2021-10-01' AND utilities_to <= '2021-10-31')
        )) );
        return $transactions->count();
    }
