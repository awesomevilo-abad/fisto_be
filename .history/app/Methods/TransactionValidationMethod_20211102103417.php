        ...
        if(strtoupper($fields['payment_type'])=="FULL"){
           ...
           if($referrence_amount == NULL){
                ...
                if(!(( $fields['referrence_group'][0]['referrence_qty'] >= $validated_full_po_transaction['po_qty']) AND ($fields['referrence_group'][0]['referrence_qty'] <= $po_allowable_qty))){
                    return TransactionValidationMethod::result(400,"Referrence Qty must be equal to PO Amount or Computed Reference amount must greater than PO Amount and less than allowable amount if allowable is triggered",null);
                }
