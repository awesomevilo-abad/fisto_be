...
class TransactionValidationMethod
 {
    ...
    public static function receiptValidation($fields,$tag_id,$date_requested,$status,$transaction_id){
        ...
        foreach($fields['po_group'] as $specific_po_details){
            ...
            $po_total_amount = $po_validation_total_amount+$po_validation_amount;
            $po_total_qty = $po_validation_total_qty+$po_validation_qty;
            $po_allowable_amount =  TransactionValidationMethod::getAllowableAmount($fields['is_allowable'],TransactionValidationMethod::convertStringToNumber($specific_po_details['po_amount']));

            if(strtoupper($referrence_type)=="DR"){
                $ref_doc_amount = TransactionValidationMethod::getDRAmount($latest_unit_price,$fields['referrence_group'][0]['referrence_qty']);

                if(!(($ref_doc_amount >= $specific_po_details['po_amount']) AND ($ref_doc_amount <= $po_allowable_amount))){
                    return TransactionValidationMethod::result(400,"Computed Referrence Amount must be equal to PO Amount or Computed Reference amount must greater than PO Amount and less than allowable amount if allowable is triggered",null);
                }
                $amount = $ref_doc_amount;
            }else{
                if(!(($referrence_amount >= $specific_po_details['po_amount']) AND ($referrence_amount <= $po_allowable_amount))){
                    return TransactionValidationMethod::result(400,"Referrence Amount must be equal to total PO Amount or Reference amount must greater than PO Amount and less than allowable amount if allowable is triggered",null);
                }
                $amount = $referrence_amount;
            }

            $get_transaction = GenericMethod::getTagIDUsingPONo($fields['payment_type'],$fields['company_id'],$fields['supplier_id'],$specific_po_details['po_no']);
            ...
            // $insertRefDetails = TransactionValidationMethod::insertRefDetails($fields,$tag_id);
            // $insertPODetails = TransactionValidationMethod::insertPODetails($validated_full_po_transaction['po_count'],$fields,$tag_id);
            return $insertTransaction = TransactionValidationMethod::insertTransaction($fields,$transaction_id,$validated_full_po_transaction['po_total_amount'],$validated_full_po_transaction['po_total_qty'],$tag_id,$date_requested,$status);
