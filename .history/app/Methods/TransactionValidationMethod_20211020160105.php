...
class TransactionValidationMethod
 {
    ...
    public static function getAllowableAmount($po_amount){
        $adjusted_po_amount = $validated_full_po_transaction['po_amount'];

        if($fields['is_allowable']){
            $allowable_amount =  $validated_full_po_transaction['po_amount']/10;
            $adjusted_po_amount =$validated_full_po_transaction['po_amount'] + $allowable_amount;
        }
    ...
    public static function receiptValidation($fields,$tag_id,$date_requested,$status,$transaction_id){
        ...
        if(strtoupper($fields['payment_type'])=="FULL"){
            ...
            if(strtoupper($referrence_type)=="DR"){
               ...
               return $po_allowable_amount =  TransactionValidationMethod::getAllowableAmount($validated_full_po_transaction['po_amount']);
