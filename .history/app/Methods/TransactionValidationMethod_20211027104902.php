...
class TransactionValidationMethod
 {
    ...
    public static function referrenceNoValidaton($payment_type,$company_id,$supplier_id,$ref_no){
        ...
        if(GenericMethod::validateIfRefNoExist($payment_type,$company_id,$supplier_id,$ref_no) > 0){
            return TransactionValidationMethod::result(
                403,"Referrence No. already exist in other Transactions",null
            );
        }
    }
    ...
    public static function insertPartialTransaction($fields,$transaction_id,$po_total_amount,$po_total_qty,$tag_id,$date_requested,$status,$po_no){
        $referrence_total_amount = null;
        $referrence_total_qty = null;
        $balance_po_ref_amount = null;
        ...
        if($balance_document_po_amount->isEmpty()){
            return 'transaction_id'."=". $transaction_id."_____"."users_id"."=". $fields['users_id']
            ."_____". "id_prefix"."=". $fields['id_prefix']
            ."_____". "id_no"."=". $fields['id_no']
            ."_____". "first_name"."=". $fields['first_name']
            ."_____". "middle_name"."=". $fields['middle_name']
            ."_____". "last_name"."=". $fields['last_name']
            ."_____". "suffix"."=". $fields['suffix']
            ."_____". "department"."=". $fields['department']
            ."_____". "document_id"."=". $fields['document_id']
            ."_____". "document_type"."=". $fields['document_type']
            ."_____". "payment_type"."=". $fields['payment_type']
            ."_____". "category_id"."=". $fields['category_id']
            ."_____". "category"."=". $fields['category']
            ."_____". "company_id"."=". $fields['company_id']
            ."_____". "company"."=". $fields['company']
            ."_____". "document_no"."=". $fields['document_no']
            ."_____". "supplier_id"."=". $fields['supplier_id']
            ."_____". "supplier"."=". $fields['supplier']
            ."_____". "document_date"."=". $fields['document_date']
            ."_____". "document_amount"."=". $fields['document_amount']
            ."_____". "remarks"."=". $fields['remarks']
            ."_____". "po_total_amount"."=". $po_total_amount
            ."_____". "po_total_qty"."=". $po_total_qty
            ."_____". "referrence_total_amount"."=".$referrence_total_amount
            ."_____". "referrence_total_qty"."=". $referrence_total_qty

            ."_____". "tag_id"."=". $tag_id
            ."_____". "tagging_tag_id"."=". 0
            ."_____". "date_requested"."=". $date_requested
            ."_____". "status"."=". $status

            ."_____". "utilities_category"."=". $fields['utilities_category']
            ."_____". "utilities_account_no"."=". $fields['utilities_account_no']
            ."_____". "utilities_consumption"."=". $fields['utilities_consumption']
            ."_____". "utilities_uom"."=". $fields['utilities_uom']
            ."_____". "utilities_receipt_no"."=". $fields['utilities_receipt_no']
            ."_____". "utilities_from"."=". $fields['utilities_from']
            ."_____". "utilities_to"."=". $fields['utilities_to']

            ."_____". "payroll_client"."=". $fields['payroll_client']
            ."_____". "payroll_category"."=". $fields['payroll_category']
            ."_____". "payroll_type"."=". $fields['payroll_type']
            ."_____". "payroll_from"."=". $fields['payroll_from']
            ."_____". "payroll_to"."=". $fields['payroll_to']

            ."_____". "pcf_date"."=". $fields['pcf_date']
            ."_____". "pcf_letter"."=". $fields['pcf_letter']

            ."_____". "balance_po_ref_amount"."=". $balance_po_ref_amount;
        }

        if(!isset($balance_document_po_amount[0]->balance_document_po_amount)){
            return TransactionValidationMethod::result(403,"PO No already exist",$po_no);
        }
        return "Meron";
    ...
    public static function getAllowableAmount($is_allowable,$po_amount){
        $adjusted_po_amount = $po_amount;

        if($is_allowable){
            $allowable_amount =  $po_amount/10;
            $adjusted_po_amount =$po_amount + $allowable_amount;
        }

        return $adjusted_po_amount;
    }
    ...
    public static function receiptValidation($fields,$tag_id,$date_requested,$status,$transaction_id){
        ...
        if(strtoupper($fields['payment_type'])=="FULL"){
            ...
            $validated_full_po_transaction = TransactionValidationMethod::fullPOValidation($fields['payment_type'],$fields['company_id'],$fields['supplier_id'],$fields['po_group']);
            $po_allowable_amount =  TransactionValidationMethod::getAllowableAmount($fields['is_allowable'],$validated_full_po_transaction['po_amount']);
            ...
            if(strtoupper($referrence_type)=="DR"){
                $ref_doc_amount = TransactionValidationMethod::getDRAmount($validated_full_po_transaction['unit_price'],$fields['referrence_group'][0]['referrence_qty']);
                if(!(($ref_doc_amount >= $validated_full_po_transaction['po_amount']) AND ($ref_doc_amount <= $po_allowable_amount))){
                    return TransactionValidationMethod::result(400,"Computed Referrence Amount must be equal to PO Amount or Computed Reference amount must greater than PO Amount and less than allowable amount if allowable is triggered",null);
                }
                $insertRefDetails = TransactionValidationMethod::insertRefDetails($fields,$tag_id);
            }else{
                if(!(($referrence_amount >= $validated_full_po_transaction['po_amount']) AND ($referrence_amount <= $po_allowable_amount))){
                    return TransactionValidationMethod::result(400,"Referrence Amount must be equal to total PO Amount or Reference amount must greater than PO Amount and less than allowable amount if allowable is triggered",null);
                }
                $insertRefDetails = TransactionValidationMethod::insertRefDetails($fields,$tag_id);
            }
        ...
        foreach($fields['po_group'] as $specific_po_details){
            ...
            $po_total_amount = $po_total_amount+$po_amount;
            $po_total_qty = $po_total_qty+$po_qty;
            $po_allowable_amount =  TransactionValidationMethod::getAllowableAmount($fields['is_allowable'],$po_amount);

            if(strtoupper($referrence_type)=="DR"){
                $ref_doc_amount = TransactionValidationMethod::getDRAmount($latest_unit_price,$fields['referrence_group'][0]['referrence_qty']);

                if(!(($ref_doc_amount >= $po_amount) AND ($ref_doc_amount <= $po_allowable_amount))){
                    return TransactionValidationMethod::result(400,"Computed Referrence Amount must be equal to PO Amount or Computed Reference amount must greater than PO Amount and less than allowable amount if allowable is triggered",null);
                }
                $amount = $ref_doc_amount;
            }else{
                if(!(($referrence_amount >= $po_amount) AND ($referrence_amount <= $po_allowable_amount))){
                    return TransactionValidationMethod::result(400,"Referrence Amount must be equal to total PO Amount or Reference amount must greater than PO Amount and less than allowable amount if allowable is triggered",null);
                }
                $amount = $referrence_amount;
            }

            $get_transaction = GenericMethod::getTagIDUsingPONo($fields['payment_type'],$fields['company_id'],$fields['supplier_id'],$po_no);
            foreach($get_transaction  as $get_specific_transaction){
                    $used_tag_id[] =  $get_specific_transaction->tag_id;
            }

            // ADDITIONAL PARTIAL TRANSACTION
            if(isset($used_tag_id)){
                return "May Laman";
            }
            // NEW PARTIAL TRANSACTION
            // $insertRefDetails = TransactionValidationMethod::insertRefDetails($fields,$tag_id);
            // $insertPODetails = TransactionValidationMethod::insertPODetails($validated_full_po_transaction['po_count'],$fields,$tag_id);
            return $insertTransaction = TransactionValidationMethod::insertPartialTransaction($fields,$transaction_id,$po_total_amount,$po_total_qty,$tag_id,$date_requested,$status,$po_no);
