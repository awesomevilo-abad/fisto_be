...
class TransactionValidationMethod
 {
    ...
    public static function insertSinglePODetails($tag_id,$po_no,$po_amount,$po_qty,$po_total_amount){
    ...
    public static function insertPartialTransaction($fields,$transaction_id,$po_total_amount,$po_total_qty,$tag_id,$date_requested,$status,$po_no){
        ...
        $balance_ref_po_amount = GenericMethod::getBalanceAmountOfDocumentPO($fields['payment_type'],$fields['company_id'],$fields['supplier_id'],$po_no,$fields['document_amount']);
        ...
        if($balance_ref_po_amount->isEmpty()){
            // First Transaction
            $balance_po_ref_amount = $po_total_amount - $referrence_total_amount;
        }

        return $balance_ref_po_amount;

        if(!isset($balance_ref_po_amount[0]->balance_ref_po_amount)){
    ...
    public static function receiptValidation($fields,$tag_id,$date_requested,$status,$transaction_id){
        ...
        foreach($fields['po_group'] as $specific_po_details){
            ...
            // Additional PO (PO With Balance)
            if(GenericMethod::validateIfPONoExistInDifferentSupplierReceiptPartial($fields['payment_type'],$fields['company_id'],$fields['supplier_id'],$po_no) > 0){
                    return "Additional PO";
            }

             // Duplicate PO
             if(GenericMethod::validateIfPONoExistInDifferentSupplier($fields['payment_type'],$fields['company_id'],$fields['supplier_id'],$po_no) > 0){
                return "PO Already exist from other transaction";
             }

            $insertPODetails = TransactionValidationMethod::insertSinglePODetails($validated_full_po_transaction['po_count'],$fields,$tag_id);
        }

            // $insertRefDetails = TransactionValidationMethod::insertRefDetails($fields,$tag_id);
            $insertTransaction = TransactionValidationMethod::insertPartialTransaction($fields,$transaction_id,$validated_full_po_transaction['po_total_amount'],$validated_full_po_transaction['po_total_qty'],$tag_id,$date_requested,$status,$po_no);
