...
class TransactionValidationMethod
 {
    ...
    public static function fullPoDocAmtValidation($po_amount,$doc_amount){
    ...
    public static function receiptValidation($fields,$tag_id,$date_requested,$status,$transaction_id){
        ...
        if(strtoupper($fields['payment_type'])=="FULL"){
            ...
            if(strtoupper($referrence_type)=="DR"){
                $doc_amount = TransactionValidationMethod::getDRAmount($validated_full_po_transaction['unit_price'],$fields['referrence_group'][0]['referrence_qty']);

                if($doc_amount != $validated_full_po_transaction['po_amount']){
                    return TransactionValidationMethod::result(400,"Computed Referrence Amount must be equal to total PO Amount",array([

                        "referrence_amount"=>$validated_full_po_transaction['po_amount'],
                        "po_amount"=>$validated_full_po_transaction['po_amount'],
                        "unit_price"=>$validated_full_po_transaction['unit_price'],
                        "computed_referrence_amount"=>$doc_amount
                    ]);
                }
            }
        }
