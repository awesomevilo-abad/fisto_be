...
class MasterlistMethod{
    ...
    public static function restore($table,$id){
        ...
        if($table == 'categories'){
                ...
                $updated_user_document_category = DB::table('user_document_categories')
                ->where('category_id', '=', $id)
                ->update(['is_active' => 1]);

                // SELECT DOCUMENT CATEGORY
                $get_document_categories = DB::table('document_categories')
                ->where('category_id', '=', $id)
                ->get();
                ...
                ->whereJsonContains('document_types->categories', [$id])
                ->get();
