...
class GenericMethod{
    ...
    public static function validateIfPayrollExist($payment_type,$company_id,
        ...
        ->where('company_id',$company_id)
        ->where('supplier_id',$supplier_id)
        ->where('payroll_client',$payroll_client)
        ...
        WHERE `company_id` = $company_id AND
        `supplier_id` = $supplier_id AND
        `payroll_client` = '$payroll_client' AND
        `payroll_category` = '$payroll_category' AND
        `payroll_type` = '$payroll_type' AND
        (
            (`payroll_from` >= '$utilities_from' AND `payroll_from` <= '$utilities_to')
                OR
            (`payroll_to` >= '$utilities_from' AND `utilities_to` <= '$utilities_to')
        )") );
