...
class TransactionValidationMethod
 {
    ...
    public static function fullPOValidation($payment_type,$company_id,$supplier_id,$po_group){
        ...
        for($i=0;$i<$po_count;$i++){
            ...
            return GenericMethod::validateIfPONoExist($payment_type,$company_id,$supplier_id,$po_no);

        }

        $result["duplicate_po"]=null;
    ...
    public static function padValidation($fields,$tag_id,$date_requested,$status,$transaction_id){
        ...
        $validated_transaction= TransactionValidationMethod::fullPOValidation($fields['payment_type'],$fields['company_id'],$fields['supplier_id'],$fields['po_group']);

        return $validated_transaction;


        if($validated_transaction['duplicate_po']){
            // return TransactionValidationMethod::result(403,"PO No already exist",null);
            return "May Laman";
        }
        return "Wala laman";

        if($fields['document_amount'] != $validated_transaction['po_total_amount']){return TransactionValidationMethod::result(400,"Document amount must be equal to total PO Amount",null);}

        $insertPODetails = TransactionValidationMethod::insertPODetails($validated_transaction['po_count'],$fields,$tag_id);

        $insertTransaction = TransactionValidationMethod::insertTransaction($fields,$transaction_id,$validated_transaction['po_total_amount'],$validated_transaction['po_total_qty'],$tag_id,$date_requested,$status);
    ...
    public static function receiptValidation($fields,$tag_id,$date_requested,$status,$transaction_id){
        ...
        if(strtoupper($fields['payment_type'])=="FULL"){
            return TransactionValidationMethod::fullPOValidation($fields['payment_type'],$fields['company_id'],$fields['supplier_id'],$fields['po_group']);
        }
            return "Partial";
    }
