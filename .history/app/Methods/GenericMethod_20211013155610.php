        ...
        $payroll_category,$payroll_type){
            ...
            ->where(function ($query) use($payroll_from,$payroll_to){
                $query->where(function ($query) use($payroll_from,$payroll_to){
                    $query->where('payroll_from','>=',$payroll_from)
                    ->where('payroll_from','<=',$payroll_to);
                })->orWhere(function ($query) use($payroll_from,$payroll_to){
                    $query->where('payroll_to','>=',$payroll_from)
                    ->where('payroll_to','<=',$payroll_to);
                });
            });
            return $transactions->count();

            $payroll_client  =  "Depot";

            $transactions = DB::select
            ( DB::raw("SELECT id FROM `transactions`
                WHERE `company_id` = $company_id AND
                `supplier_id` = $supplier_id AND
                JSON_CONTAINS(`payroll_client`,'$payroll_client')
                `payroll_category` = '$payroll_category' AND
                `payroll_type` = '$payroll_type' AND
                (
                    (`payroll_from` >= '$payroll_from' AND `payroll_from` <= '$payroll_to')
                        OR
                    (`payroll_to` >= '$payroll_from' AND `payroll_to` <= '$payroll_to')
                )"
            ) );
            return count($transactions);
    }
