        ...
        if(strtoupper($fields['payment_type'])=="FULL"){
            ...
            if(strtoupper($referrence_type)=="DR"){
                ...
                if(!(($ref_doc_amount >= $validated_full_po_transaction['po_amount']) AND ($ref_doc_amount <= $po_allowable_amount))){
                    return TransactionValidationMethod::result(400,"Computed Referrence Amount must be equal to PO Amount or Reference amount must greater than PO Amount and less than allowable amount if allowable is triggered",null);
