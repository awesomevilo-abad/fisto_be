...
class GenericMethod{
    ...
    $utilities_receipt_no){
        ...
        ->where('company_id',$company_id)
        ->where('supplier_id',$supplier_id)
        ->where('utilities_category',$utilities_category)
        ->where('utilities_account_no',$utilities_account_no)
        ->where(function($query) use($utilities_from,$utilities_to){
            $query->where('utilities_from','>=',$utilities_from)
            ->orWhere('utilities_to','<=',$utilities_from);
        })->orWhere(function($query) use($utilities_from,$utilities_to){
            $query->where('utilities_from','>=',$utilities_to)
            ->orWhere('utilities_to','<=',$utilities_to);
        })

        ->where('utilities_to','<=',$utilities_to);
        return $transactions->count();
    }

    public static function validateIfPayrollExist($payment_type,$company_id,
    $supplier_id,$payroll_from
    ,$payroll_to,$payroll_location,
    $payroll_tax_category){
        $transactions = DB::table('transactions')
        ->where('company_id',$company_id)
        ->where('supplier_id',$supplier_id)
        ->where('payroll_location',$payroll_location)
        ->where('payroll_tax_category',$payroll_tax_category)
        ->where('payroll_from','>=',$payroll_from)
