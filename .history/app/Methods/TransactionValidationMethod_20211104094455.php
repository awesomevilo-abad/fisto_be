    ...
    public static function receiptValidation($fields,$tag_id,$date_requested,$status,$transaction_id){
        ...
        if(isset($existing_po)){
            ...
            if(isset($additional_po)){
                ...
                foreach($fields['po_group'] as $specific_po_details){
                    foreach($additional_po as $specific_additional_po){
                        ...
                        if($specific_additional_po == $specific_po_details['po_no']){
                            ...
                            $additional_po_total_qty= $additional_po_total_qty + TransactionValidationMethod::convertStringToNumber($specific_po_details['po_qty']);
                        }
                    }
                }
                ...
                if((strtoupper($referrence_type)=="DR") AND ($referrence_amount == NULL)){
                    $balance_ref_po_qty = GenericMethod::getBalanceQtyOfRefPO($fields['payment_type'],$fields['company_id'],$fields['supplier_id'],end($duplicate_po),$fields['referrence_group'][0]['referrence_qty']);
                    $additional_po_qty_plus_balance =  $additional_po_total_qty + $balance_ref_po_qty[0]->balance_po_ref_qty;

                    if($additional_po_qty_plus_balance == 0 ){
                        return TransactionValidationMethod::result(400,"Transaction, PO Closed or PO belong to other batch",array(["total_po_qty"=>$additional_po_qty_plus_balance]));
                    }

                    if($additional_po_qty_plus_balance < $fields['referrence_group'][0]['referrence_qty'] ){
                        return TransactionValidationMethod::result(400,"Referrence Qty is greater than Total PO Qty, Additional PO Required",array(["referrence_qty"=>$fields['referrence_group'][0]['referrence_qty'], "total_po_qty"=>$additional_po_qty_plus_balance]));
                    }
                    ...
                    if($additional_po_amount_plus_balance == 0 ){
                        return TransactionValidationMethod::result(400,"Transactions, PO Closed or PO belong to other batch",array(["total_po_amount"=>$additional_po_amount_plus_balance]));
                    }
                ...
                $balance_po_ref_amount = null;
                $balance_po_ref_qty = null;

                if(isset($additional_po_amount_plus_balance)){
                    $balance_po_ref_amount =  $additional_po_amount_plus_balance - $referrence_amount;
                }
                if(isset($additional_po_qty_plus_balance)){
                    $balance_po_ref_qty =  $additional_po_qty_plus_balance - $fields['referrence_group'][0]['referrence_qty'];
                }

                $insertRefDetails = TransactionValidationMethod::insertRefDetails($fields,$tag_id);
                $insertPODetails = TransactionValidationMethod::insertPODetails(count($fields['po_group']),$fields,$tag_id);
                $insertPartialTransaction = TransactionValidationMethod::insertPartialTransaction($fields,$transaction_id,$po_total_amount,$po_total_qty,$tag_id,$date_requested,$status,$po_no,$balance_po_ref_amount,$balance_po_ref_qty);
