...
public static function prmValidation($fields,$tag_id,$date_requested,$status,$transaction_id){
    ...
    $documentNoValidation = $documentNoValidation ?? "default";
    $fields = $fields ?? "default";
    $transaction_id = $transaction_id ?? "default";
    $po_total_amount = $po_total_amount ?? "default";
    $po_total_qty = $po_total_qty ?? "default";
    $tag_id = $tag_id ?? "default";
    $date_requested = $date_requested ?? "default";
    $status = $status ?? "default";

    if(!isset($po_total_amount)){
        $po_total_amount=null;
    }
    if(!isset($po_total_qty)){
        $po_total_qty=null;
    }
    if(!isset($tag_id)){
        $tag_id=null;
    }
    if(!isset($date_requested)){
        $date_requested=null;
    }
    if(!isset($status)){
        $status=null;
