            ...
            foreach($fields['po_group'] as $specific_po_details){
                foreach($additional_po as $specific_additional_po){
                    if($specific_additional_po == $specific_po_details['po_no']){
                        $balance_ref_po_amount = GenericMethod::getBalanceAmountOfRefPO($fields['payment_type'],$fields['company_id'],$fields['supplier_id'],$po_no,end($duplicate_po));
                        return $fields['company_id']."_".$fields['supplier_id']."_".end($duplicate_po);
