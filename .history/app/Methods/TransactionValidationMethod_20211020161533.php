    ...
    public static function receiptValidation($fields,$tag_id,$date_requested,$status,$transaction_id){
        ...
        if(strtoupper($fields['payment_type'])=="FULL"){
            ...
            if(strtoupper($referrence_type)=="DR"){
               ...
               return $po_allowable_amount =  TransactionValidationMethod::getAllowableAmount($fields['is_allowable'],$validated_full_po_transaction['po_amount']);

               if($ref_doc_amount != $validated_full_po_transaction['po_amount']){
                return TransactionValidationMethod::result(400,"Computed Referrence Amount must be equal to total PO Amount",null);
            }

                if(($ref_doc_amount >= $validated_full_po_transaction['po_amount']) OR ($ref_doc_amount >= $validated_full_po_transaction['po_amount'])){
                    return TransactionValidationMethod::result(400,"Computed Referrence Amount must be equal to total PO Amount",null);
                }
