...
class DocumentController extends Controller
{
    public function index(Request $request,$status,$tableRows)
    {
        ...
        foreach ($category_ids as $specific_document) {
            ...
            $document_details = DB::table('documents AS d')
                ->select('id', 'document_type', 'document_description', 'is_active','updated_at','deleted')
                ->where('is_active', '=', 1)
                ->where('id', '=', $doc_id)
                ->get();
                ...
                "updated_at"=>$document_details[0]->document_type,
                "deleted_at"=>$document_details[0]->document_description]);
        }
    ...
    public function update(Request $request, $id)
    {
        ...
        if(count($validateDuplicateDocumentTypeInUpdate)>0) {
            $code =403;
            $message = "Document Already Registered in other Document Type";
            $data = [];
            return $this->result($code,$message,$data);
        }
