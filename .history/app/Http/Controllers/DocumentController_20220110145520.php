...
class DocumentController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        foreach ($request['categories'] as $cat) {
            ...
            array_push($category_id_list,$cat['id']);
            if ($active_categories->contains($cat['id'])) {
                $category_details->push(['category_is_active' => 1, 'cat_id' => $cat['id']]);
            } else {
                $category_details->push(['category_is_active' => 0, 'cat_id' => $cat['id']]);
            }
        }
    ...
    public function search(Request $request,$status,$tableRows)
    {
        ...
        $category_object = collect();
        foreach($result as  $getCategories){
            $category_object->push(['id'=>$getCategories->cat_id,
            'name'=>$getCategories->cat_name]);
        }
        ...
        foreach ($category_ids as $specific_document) {
            ...
            $document_details = DB::table('documents AS d')
                ...
                "categories"=>$cat_id->unique()]);


        }

        $document_with_pagination= GenericMethod::paginateme($documents,$tableRows);
        if (!$document_details || $document_details->isEmpty()) {
            ...
            $code =    200;
            $message = "Succefully Retrieved";
            $data = $document_with_pagination->unique();
        }
