...
class DepartmentController extends Controller
{
    ...
    public function store(Request $request){
        ...
        $validate_department_department = DB::table('departments')
        ->where('department',$fields['department'])
        ->where('department',$fields['department'])->get();

        if(count($validate_department_department)>0){
    ...
    public function update(Request $request, $id)
    {
        ...
        if (!$specific_department) {
            return $this->result(404,'Data Not Found')
        } else {

            $specific_department->department_code = $request->get('code');
            $specific_department->department_description = $request->get('department');
